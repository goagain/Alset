...
class speedsign_detector:
    def __init__(self, img_size=640, stride=32, debug = -1):
        path = 'foo'
        opt_device = '0'
        weights = 'speedsign_detector.pt'
    ...
    def detect(self, image, frame=12345):
        ...
        bgr_image = newimg_np #cv2.cvtColor(newimg_np, cv2.COLOR_RGB2BGR)
        im0s = bgr_image
        if self.debug>0:
            print(f'im0s shape:',im0s.shape)
        img = im0s[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB
        if self.debug>0:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    self.curlimitspeed = self.names[int(cls)]
                    bchange = True
            ...
            return bgr_image,bchange, self.curlimitspeed