...
def apply_color_threshold(image):
    ...
    s_channel = hls[:,:,2]
    s_thresh_min = 170.0/255.0
    s_thresh_max = 255.0/255.0
...
def compare_plotted_images(image1, image2, image1_exp="Image 1", image2_exp="Image 2"):
    f, (ax1, ax2) = plt.subplots(1, 2, figsize=(24, 9))
    f.tight_layout()
    x = float(image1.shape[1])
    y = float(image1.shape[0])    
    ax1.imshow(image1)
    ax1.plot([0.55*x, 0.85*x], [0.55*y, 0.86*y], color='r', linewidth="5")
    ax1.plot([0.85*x, 0*x], [0.86*y, 0.86*y], color='r', linewidth="5")
    ax1.plot([0*x, 0.39*x], [0.86*y, 0.55*y], color='r', linewidth="5")
    ax1.plot([0.39*x, 0.55*x], [0.55*y, 0.55*y], color='r', linewidth="5")
    ax1.set_title(image1_exp, fontsize=50)
    ax2.imshow(image2)
    ax2.plot([0.95*x, 0.95*x], [0, 0.97*y], color='r', linewidth="5")
    ax2.plot([0.95*x, 0.0*x], [0.97*y, 0.97*y], color='r', linewidth="5")
    ax2.plot([0.0*x, 0.0*x], [0.97*y, 0], color='r', linewidth="5")
    ax2.plot([0.0*x, 0.95*x], [0, 0], color='r', linewidth="5")
    ax2.set_title(image2_exp, fontsize=50)
    plt.subplots_adjust(left=0., right=1, top=0.9, bottom=0.)
...
def slide_window(binary_warped, histogram):
    x = float(binary_warped.shape[1])
    y = float(binary_warped.shape[0])
...
#     print(newwarp.shape)
    if (original_image.shape[2]==4):
        newwarp = cv2.cvtColor(newwarp, cv2.COLOR_RGB2BGRA)
    if (newwarp.dtype != original_image.dtype):
        newwarp = newwarp.astype(original_image.dtype)
#     print(original_image.shape)
#     print(newwarp.shape)
...
def process_lane_detect(img_np):
    global used_warped
    global used_ret

    array = np.frombuffer(img_np.raw_data, dtype=np.dtype("uint8"))
    array = np.reshape(array, (img_np.height, img_np.width, 4))
    ...
    cv2.putText(result, curvature_text, (10, 30), fontType, 0.8, (255, 255, 255), 3)
    ...
    cv2.putText(result, deviation_text, (10, 60), fontType, 0.8, (255, 255, 255), 3)
